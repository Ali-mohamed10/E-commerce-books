...
const Footer = () => {
  return (
    <footer className="bg-gradient-to-r from-[#ddd]/10 via-[#ddd]/10 to-second dark:from-black dark:via-black dark:to-second text-gray-900 dark:text-white py-12 px-4 font-inter border-t border-main">
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <div className="space-y-4">
          ...
          <div className="flex space-x-5 pt-2">
            <a
              href="#"
              className="text-gray-500 hover:text-second transition-transform transform hover:scale-110"
            >
              <GitHubIcon size={28} />
            </a>
            <a
            ...
            {links.map((link, index) => {
              return (
                <Link to={link.to} key={index}>
                  <li
                    className={`hover:text-hover transition duration-300 cursor-pointer pb-2 w-fit
      ...
      <div className="text-center text-gray-500 dark:text-gray-400 text-sm pt-10 mt-10 border-t border-second/30">
        <p>&copy; {new Date().getFullYear()} NYT. All rights reserved.</p>
        <p className="mt-1">
          Designed with <span className="text-red-500">&hearts;</span> by NYT
          Books
        </p>
      </div>
    </footer>
